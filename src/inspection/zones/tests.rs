...
fn zone_detail_adds_compact_facts_without_repeating_summary_sections() -> Result<()> {
    ...
    let detail = ZoneDetail {
        ...
        source_agreement: ZoneSourceAgreement {
            state: SequencerAgreementState::SingleSource,
            comparable_source_count: 1,
            eligible_source_ids: vec!["seq-primary".to_owned()],
            compared_block_ids: vec![],
            lagging_source_ids: vec![],
            divergent_block_ids: vec![],
            finalized_conflict_source_ids: vec![],
            unverified_block_ids: vec![],
        },
