...
pub(super) fn apply_event(state: &mut LocalNodesState, event: &ModuleLifecycleEvent) -> bool {
    let Some(record) = state.topology_mut(&event.target.network_id) else {
        return false;
    };
...
mod tests {
    ...
    fn runtime_restart_preserves_generated_bedrock_config_for_direct_start() -> Result<()> {
        ...
        let policy = adapter_for(NodeKind::Bedrock).action_policy(NodeAction::Start);
        if !node.installed
    ...
    fn state_with_pending(target: LifecycleTarget) -> LocalNodesState {
        LocalNodesState {
            ...
            devnets: vec![super::super::model::LocalDevnetRecord {
                deployment: super::super::model::LocalNodeDeployment::LocalDevnet,
